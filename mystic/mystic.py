...
from mystic import PenguinStringCompiler
from mystic.data import db
from mystic.data.permission import PermissionCollection
...
import mystic.handlers
import mystic.plugins

from mystic.handlers import XTListenerManager, XMLListenerManager, DummyEventListenerManager
from mystic.plugins import PluginManager
from mystic.commands import CommandManager
...
class Mystic:
    ...
    async def start(self):
        ...
        if self.config.type == 'world':
            ...
            await self.xml_listeners.setup(mystic.handlers, exclude_load='mystic.handlers.login.login')
            await self.xt_listeners.setup(mystic.handlers)
            self.logger.info('World server started')
        else:
            await self.xml_listeners.setup(mystic.handlers, 'mystic.handlers.login.login')
