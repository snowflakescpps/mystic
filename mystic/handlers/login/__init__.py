...
from mystic.converters import VersionChkConverter
from mystic.data.buddy import BuddyList
...
async def get_server_presence(p, pdata):
    ...
    pops = await p.server.redis.hgetall('mystic.population')
    for server_id, server_population in pops.items():
        ...
        server_key = f'mystic.players.{int(server_id)}'
