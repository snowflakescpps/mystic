...
from mystic.converters import Credentials
from mystic.crypto import Crypto
from mystic.data.moderator import Ban
from mystic.data.penguin import Penguin
from mystic.handlers import XMLPacket
from mystic.handlers.login import get_server_presence
from mystic.handlers.play.navigation import get_minutes_played_today
...
async def handle_login(p, credentials: Credentials):
    ...
    if p.client_type == ClientType.Vanilla:
        raw_login_data = f'{data.id}|{data.id}|{data.username}|{login_key}|mystic|{data.approval}|{data.rejection}'
        if not data.active:
