...
async def handle_rainbow_puffle_task_complete(p, task_id: int):
    ...
    current_task = await p.server.redis.get(f'mystic.rainbow_task.{p.id}') or 0

    if int(current_task) < len(RainbowQuestRewards) and int(current_task) == task_id:
        task_completion = await p.server.redis.get(f'mystic.rainbow_completion.{p.id}') or time.time()
        ...
        if int(current_task) == 0 or current_datetime - datetime.fromtimestamp(int(task_completion)) > quest_wait:
            ...
            await p.server.redis.incr(f'mystic.rainbow_task.{p.id}')
            await p.server.redis.set(f'mystic.rainbow_completion.{p.id}', int(time.time()))
            await p.server.redis.expireat(f'mystic.rainbow_task.{p.id}', progression_expiry)
            await p.server.redis.expireat(f'mystic.rainbow_completion.{p.id}', progression_expiry)
...
async def handle_rainbow_quest_cookie(p):
    ...
    if current_task == len(RainbowQuestRewards) and not p.rainbow_adoptability:
        await p.server.redis.delete(f'mystic.rainbow_task.{p.id}',
                                    f'mystic.rainbow_completion.{p.id}',
                                    f'mystic.rainbow_coins.{p.id}')
        current_task = 0

    current_datetime = datetime.now()
    task_completion = await p.server.redis.get(f'mystic.rainbow_completion.{p.id}')
    coins_collected = await p.server.redis.smembers(f'mystic.rainbow_coins.{p.id}', encoding='utf-8')
...
async def handle_rainbow_puffle_task_coin_collected(p, task_id: int):
    current_task = await p.server.redis.get(f'mystic.rainbow_task.{p.id}')
    coins_collected = await p.server.redis.sismember(f'mystic.rainbow_coins.{p.id}', task_id)
    if task_id <= int(current_task) and not coins_collected:
        await p.server.redis.sadd(f'mystic.rainbow_coins.{p.id}', task_id)
        await p.server.redis.expireat(f'mystic.rainbow_coins.{p.id}',
...
async def handle_rainbow_puffle_task_bonus_collected(p):
    if p.rainbow_adoptability:
        if RainbowBonusReward not in p.inventory:
            ...
            await p.server.redis.sadd(f'mystic.rainbow_coins.{p.id}', 'bonus')
        else:
            coins_collected = await p.server.redis.sismember(f'mystic.rainbow_coins.{p.id}', 'bonus')
