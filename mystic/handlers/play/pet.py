...
from mystic.data.mail import PenguinPostcard
from mystic.data.pet import PenguinPuffle, PenguinPuffleCollection, PenguinPuffleItemCollection, PuffleCollection, \
    PuffleItemCollection, PuffleTreasureFurniture, PuffleTreasureItem, PuffleTreasurePuffleItem
from mystic.data.room import PenguinBackyardRoom, PenguinIglooRoom
from mystic.handlers import Priority, XMLPacket, XTPacket
...
async def dig(p, on_command=False):
    if p.walking is not None:
        ...
        await p.server.redis.setex(f'mystic.last_dig.{p.id}', 120, int(time.time()))
        dig_count = await p.server.redis.incr(f'mystic.dig_count.{p.id}')

        if dig_count == 1:
            await p.server.redis.expireat(f'mystic.dig_count.{p.id}',
...
async def handle_get_dig_cooldown(p):
    last_dig = await p.server.redis.get(f'mystic.last_dig.{p.id}')
